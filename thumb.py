...
def _overlay_gradient(img, color, dari="kiri",
    ...
    """
    Gradient HORIZONTAL (kiri/kanan).
    FIX: loop i in range(W), garis vertikal per kolom.
    """
...
def _overlay_gradient_vertikal(img, color, dari="bawah",
    ...
    FIX: loop i in range(H), garis horizontal per baris.
    """
    from PIL import Image, ImageDraw
    ov = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    od = ImageDraw.Draw(ov)
...
def _tmpl_ch1(info, judul, output_path):
    ...
    if st == "Naik":
        bg_badge = (255, 220, 100)
        tc_badge = (80, 50, 0)
        icon     = "▲ NAIK"
    elif st == "Turun":
        bg_badge = (255, 180, 120)
        tc_badge = (80, 30, 0)
        icon     = "▼ TURUN"
    else:
        bg_badge = (220, 240, 180)
        tc_badge = (40, 60, 10)
        icon     = "= STABIL"
    ...
    draw_text_stroke(draw, W - 30, 28,
                     NAMA_CHANNEL, get_font(fp, 26),
                     (255, 235, 130),
                     stroke=3, stroke_fill=(80, 40, 0),
                     anchor="rt")

    # ── Harga besar kiri bawah ──
    draw_text_stroke(draw, 34, H - 230,
                     "Harga per gram",
                     get_font(fp, 30),
                     (255, 230, 160),
...
def _tmpl_ch2(info, judul, output_path):
    ...
    # ── Badge status kiri atas ──
    if st == "Naik":
        bg_badge = (160, 230, 255)
        tc_badge = (0, 50, 100)
        icon     = "▲ NAIK"
    elif st == "Turun":
        bg_badge = (180, 210, 255)
        tc_badge = (20, 20, 100)
        icon     = "▼ TURUN"
    else:
        bg_badge = (200, 240, 255)
        tc_badge = (0, 60, 100)
        icon     = "= STABIL"
...
def _tmpl_ch5(info, judul, output_path):
    ...
    # FIX: buat card di canvas penuh dulu, baru composite
    # (jangan composite langsung dengan ukuran berbeda)
    card_w   = W - 100
    card_h   = 300
    card_x   = 50
    card_y   = H // 2 - 150
